...
io.on('connection', (socket) => {
    ...
    socket.on('init_user', (data) => {
        users[socket.id] = { ...data, ip: ip, sid: socket.id };
        console.log(`[CONNEXION] ${data.name} - IP: ${ip}`);
        io.emit('update_users', Object.values(users));
    });

    // CHAT PUBLIC
    socket.on('chat_msg', (data) => {
        if (data.text && data.text.startsWith("/bomb ")) {
    ...
    // MESSAGE PRIVÉ (DM)
    socket.on('private_msg', ({ toSid, text, fromName, fromPP }) => {
        if(users[toSid]) {
            io.to(toSid).emit('private_msg', { text, fromName, fromPP, fromSid: socket.id });
        }
    });
    ...
    socket.on('call_user', (toSid) => {
        if(users[toSid]) {
            io.to(toSid).emit('incoming_call', { 
                fromName: users[socket.id].name, 
                fromPP: users[socket.id].pp,
                fromSid: socket.id 
            });
        }
    });
...
http.listen(PORT, () => console.log(`PRISMA_V20_ONLINE`));
